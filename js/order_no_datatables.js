$(document).ready(function() {
    ...
    var order_table = $('#order-table').dataTable({
		...
		"aaSorting": [],
		"iDisplayLength" : 15,
		"iDisplayStart"  : parseInt(document.location.hash.substr(1).match(/^[0-9]+$/) ? document.location.hash.substr(1) : 0),
		"aoColumnDefs": [
   ...
   // var bVis = oTable.fnSettings().aoColumns[iCol].bVisible;
	...
	$.each(hidden_columns, function(index, value) {
		
		if (index = $('#order-table thead tr:first th[data-name="' + value + '"]').index()) {
			indexes.push(index);
		}
	});

	$.each(indexes, function(index, value) {
		order_table.fnSetColumnVis( value, false);
	});
