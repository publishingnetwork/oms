...
class Controller_System extends Controller_Check {
	...
	public function action_scrape_fullfillment_aus() {
			...
			if ($this->request->post('type') != 'paypal') {
				...
				if (!$order->loaded()) {
					...
					if ($paypal_details->loaded()) {
						...
						$aus_fulfilment = false;
						$products_to_orders = ORM::factory('ProductsToOrder')
							->where('order_id', '=', $paypal_details->order_id)
							->find_all();
...
*/
	public function action_manual_download_paypal() {
		...
		$search_request_type = new TransactionSearchRequestType();
		$search_request_type->StartDate = date("Y-m-d\TH:i:sO", strtotime('-1 day 00:00'));
